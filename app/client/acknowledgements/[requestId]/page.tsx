...
import { getPortalAcknowledgementRequestById, markAcknowledgementViewed, submitAcknowledgementByPortalId } from "@/lib/services/esign/client-acknowledgement";
import { PortalCard, PortalSectionHeading, PortalShell, PortalStatusBadge } from "@/components/client-portal/portal-ui";

async function submitRequest(portalId: string, requestId: string, formData: FormData) {
  "use server";
  const headerStore = await headers();
  const ip = headerStore.get("x-forwarded-for")?.split(",")[0]?.trim() || headerStore.get("x-real-ip") || "unknown-ip";
  const userAgent = headerStore.get("user-agent");
  const limit = checkRateLimit({ key: `portal.acknowledgement.request:${ip}:${requestId}`, limit: 6, windowMs: 10 * 60 * 1000 });
  if (!limit.allowed) redirect(`/client/acknowledgements/${requestId}?error=rate-limited`);
  try {
    await submitAcknowledgementByPortalId({
      portalId,
      requestId,
      formData,
      clientIp: ip,
    ...
    redirect(`/client/acknowledgements/${requestId}?submitted=1`);
  } catch (error) {
    const message = error instanceof Error ? error.message : "Unable to submit acknowledgement.";
    redirect(`/client/acknowledgements/${requestId}?error=${encodeURIComponent(message)}`);
  }
}

export default async function ClientAcknowledgementPage({
  params,
  searchParams
}: {
  ...
  const request = await getPortalAcknowledgementRequestById(portal.id, params.requestId);
  if (!request?.definition) notFound();
  await markAcknowledgementViewed({ portalId: portal.id, requestId: request.id }).catch(() => null);
  const handleSubmit = submitRequest.bind(null, portal.id, request.id);
        ...
        <PortalCard>
          <form action={handleSubmit} className="space-y-5">
            ...
            <div className="flex flex-wrap items-center justify-between gap-3">
              <Link href={"/client/portal" as any} className="text-sm font-medium text-violet-700 hover:text-violet-900">Back to portal</Link>
